...
RobStrite_Motor RobStrite03[] = {
    RobStrite_Motor(0x01, &hfdcan1,-1,2.5307f-0.00133514404f),  // 第1个关节电机
    RobStrite_Motor(0x02, &hfdcan1,-1,0.6108f+6.2212944f),  // 第2个关节电机
    RobStrite_Motor(0x01, &hfdcan2,1,2.5307f-6.2579155f),  // 第3个关节电机
	RobStrite_Motor(0x02, &hfdcan2,1,0.6108f-0.0108718872f)   // 第4个关节电机
};
/*******************************************************************************
...
void RobStrite_Motor::RobStrite_Motor_Analysis(uint8_t *DataFrame,uint32_t ID_ExtId)
{
	...
	{		
		...
		{
			Pos_Info.Angle =  Rota_dir*uint16_to_float(DataFrame[0]<<8|DataFrame[1],P_MIN,P_MAX,16) + ang_offset;
			Pos_Info.Speed =  Rota_dir*uint16_to_float(DataFrame[2]<<8|DataFrame[3],V_MIN,V_MAX,16);			
			Pos_Info.Torque = Rota_dir*uint16_to_float(DataFrame[4]<<8|DataFrame[5],T_MIN,T_MAX,16);				
		...
		}else if (int((ID_ExtId&0x1F000000)>>24) == 21)
		{
			error_cnt++;
			memcpy(error_data,DataFrame,8);
		}
...
{
	Motor_Set_All.set_current = Rota_dir*current;
